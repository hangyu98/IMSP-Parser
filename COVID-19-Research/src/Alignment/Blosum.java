...
public class Blosum {
    ...
    private static int getIndex(char a) {
        ...
        switch ((String.valueOf(a)).toUpperCase().charAt(0)) {
            ...
            case 'V':
                return 19;
            default:
                throw new IllegalArgumentException("Invalid amino acid character");
        }
    }

    /**
     * get score from the BLOSUM62 matrix
     * @param i x index
     * @param j y index
     * @return score
     */
    private static int getDistance(int i, int j) {
        if (i < 0 || i > blosum62[0].length) {
            throw new IllegalArgumentException("Invalid amino acid character at string1, position " + i);
        }
        if (j < 0 || j > blosum62[0].length) {
    ...
    public static void main(String[] args) {
        System.out.println(getDistance('A', 'R'));
    }
}